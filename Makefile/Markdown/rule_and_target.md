...
```
 ...
 2. **Order-Only依赖**：**目标文件** 根据 **依赖文件** 中的部分来决定是否需要被重建，而不是在任何一个 **依赖文件** 被修改后都重建 **目标文件**。
...
```
 ...
 - 目标 `FORCE` 为一个 **强制目标**，并作为目标 `target` 的依赖。

 - 在执行 `make target` 时，由于 `FORCE` 被认为总是需要更新，所以 `target` 所在的规则总会被执行。
